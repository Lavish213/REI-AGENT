...
async def handle_outbound_answered(
    ...
    try:
        ...
        if answered_by in _VM_MACHINE_TYPES:
            logger.info(
                "outbound_voicemail_detected "
                "lead_id={} "
                "answered_by={}",
                lead_id,
                answered_by,
            ...
            laml = await _build_voicemail_response(
                lead,
                prop,
            )
        ...
        laml = build_outbound_answer_laml(
            lead_id,
        )

        logger.info(
            "outbound_answered_connecting_stream "
            "lead_id={}",
            lead_id,
        )
...
async def outbound_voice_stream(
    ...
    try:
        ...
        prop = (
            lead.get("properties") or {}
            if lead
            else {}
        )

        first_name = (
            _get_first_name(lead, prop)
            if lead
            else "there"
        )
        ...
        landmark = (
            await _get_landmark_for_property(prop)
            if prop
            else None
        )
        ...
        prior_context = (
            build_prior_call_context(lead)
            if lead
            else None
        )

        base_context_str = (
            _build_outbound_context(
                lead,
                prop,
                first_name,
                address,
...
async def _build_voicemail_response(
    lead: dict,
    prop: dict,
) -> str:
    try:
        in_stl = (
            not lead.get(
                "speed_to_lead_completed",
                False,
            )
            and (
                lead.get(
                    "speed_to_lead_attempts"
                )
                or 0
            ) > 0
        )
        ...
        if in_stl:
            ...
            script_num = select_script_number(
                stl_attempts
            )

            logger.info(
                "ringless_voicemail_selected "
                "lead_id={} "
                "script={} "
                "stl_attempts={}",
                lead.get("id"),
            ...
            return build_ringless_voicemail_laml(
                lead,
                prop,
                script_num,
            )

        return build_voicemail_laml(
            lead,
            prop,
        )
        ...
        logger.exception(
            "build_voicemail_response_failed "
            "lead_id={} "
            "error={}",
            lead.get("id"),
            str(e),
...
async def _get_landmark_for_property(
    prop: dict,
) -> str | None:
    try:
        address = prop.get(
...
def _build_outbound_context(
    ...
    if landmark:
        neighborhood_line = (
            ...
            f'Use naturally: "That whole area '
            f'near {landmark} has been moving '
            f'really fast lately."'
        )

    else:
        neighborhood_line = (
            f'Use city hook naturally: '
            f'"That whole {city} area '
            f'has been really active lately."'
        )
